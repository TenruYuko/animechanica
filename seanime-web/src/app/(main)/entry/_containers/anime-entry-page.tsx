...
import { FamilyTreeSection, FamilyTreeData } from "@/app/(main)/entry/_components/family-tree-section"
import useSWR from "swr"
// If swr is not installed, run: npm install swr
...
export function AnimeEntryPage() {
    ...
    const { data: familyTreeData } = useSWR<FamilyTreeData>(
        mediaId ? `/api/v1/family-tree/${mediaId}` : null,
        async (url: string): Promise<FamilyTreeData> => {
            const res = await fetch(url)
            if (!res.ok) throw new Error("Failed to fetch family tree")
            return res.json() as Promise<FamilyTreeData>
        }
    )
    ...
    }, [animeEntry, animeEntryLoading, router])
    ...
    {isLibraryView && (
      <PageWrapper
        data-anime-entry-page-library-view
        key="library-episodes"
        className="relative 2xl:order-first pb-10 lg:pt-0"
      >
        <EpisodeSection entry={animeEntry} details={animeDetails} bottomSection={null} />
        <MetaSection entry={animeEntry} details={animeDetails} />
        <MediaEntryCharactersSection details={animeDetails} />
        <FamilyTreeSection data={familyTreeData} />
