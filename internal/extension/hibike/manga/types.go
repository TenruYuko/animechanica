...
type (
	Provider interface {
		...
		// FindVolumes returns the volume details for the given manga ID.
		FindVolumes(id string) ([]*VolumeDetails, error)
	...
	VolumeDetails struct {
		// "ID" of the extension.
		Provider string `json:"provider"`
		// ID of the volume, used to fetch the volume's chapters.
		// It should be unique within the provider.
		ID string `json:"id"`
		// The volume number (e.g. "1", "2", etc)
		Number string `json:"number"`
		// The volume title if any (e.g. "The Beginning", etc)
		Title string `json:"title,omitempty"`
		// List of chapters in this volume
		Chapters []*ChapterDetails `json:"chapters"`
		// Cover image URL for this volume if available
		CoverImage string `json:"coverImage,omitempty"`
		// Release date of this volume in YYYY-MM-DD format
