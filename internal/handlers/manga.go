...
// HandleGetMangaEntryVolumes
//
//      @summary returns volumes for a manga entry from a provider.
//      @desc This endpoint returns volume information including cover images and chapter listings.
//      @route /api/v1/manga/entry/{id}/volumes [GET]
//      @param id - int - true - "AniList manga media ID"
//      @returns manga.VolumeContainer
func (h *Handler) HandleGetMangaEntryVolumes(c echo.Context) error {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		return h.RespondWithError(c, err)
	}

	// Get the manga entry to get titles and year
	anilistEntry, err := h.App.AnilistPlatform.GetManga(id)
	if err != nil {
		return h.RespondWithError(c, err)
	}

	// Get the current provider from query param
	...
	opts := &manga.GetMangaChapterContainerOptions{
		Provider: provider,
		MediaId:  id,
		Titles:   anilistEntry.GetAllTitles(),
		Year:     anilistEntry.GetStartYearSafe(),
	}

	volumeContainer, err := h.App.MangaRepository.GetMangaVolumeContainer(opts)
	if err != nil {
		return h.RespondWithError(c, err)
	}

	return h.RespondWithData(c, volumeContainer)
}
