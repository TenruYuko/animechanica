...
// getAniListTokenFromCookieOrDefault returns the AniList token from the user's cookie, or the provided default if not present
func getAniListTokenFromCookieOrDefault(c echo.Context, fallback string) string {
	cookie, err := c.Cookie("AniList-Token")
...
func (h *Handler) HandleAnilistListAnime(c echo.Context) error {
	...
	ret, err := anilist.ListAnimeM(
		...
		&isAdult,
		h.App.Logger,
		getAniListTokenFromCookieOrDefault(c, h.App.GetAccountToken()),
	)
	if err != nil {
		return h.RespondWithError(c, err)
	}
