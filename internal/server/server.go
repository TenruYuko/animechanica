...
func startApp(embeddedLogo []byte) (*core.App, core.SeanimeFlags, *updater.SelfUpdater) {
	// Load anime-offline-database.json for family tree lookups
	if err := anime.LoadAnimeOfflineDatabase("external/anime-offline-database.json"); err != nil {
		golog.Printf("[WARN] Could not load anime-offline-database.json: %v\n", err)
	}
	// Print the header
	core.PrintHeader()
