...
func CompareWithSorensenDice(v *string, vals []*string) []*SorensenDiceResult {
	...
	dice.NgramSize = 1 // More lenient ngram size for better partial matches

	res := make([]*SorensenDiceResult, len(vals))

	// Normalize strings by converting to lowercase and removing special characters
	normalize := func(s string) string {
		// Convert to lowercase and remove punctuation/special chars except spaces
	...
	originalNorm := normalize(*v)
	for _, val := range vals {
		valNorm := normalize(*val)
		res = append(res, &SorensenDiceResult{
			OriginalValue: v,
			Value:         val,
			Rating:        dice.Compare(originalNorm, valNorm),
		})
	}
